...
#### PITCH ESTIMATION ####
def find_spectral_peaks_prominence(y, vmask, fs, hop_length, win_length, n_fft, f0_min=300.0, f0_max=8000.0,
                                   noise_margin_db=6.0, min_prominence_db=10.0):
    """
    Find prominent spectral peaks for each frame
    """
    # compute STFT
...
def get_pitch(y, vmask, fs, hop_length, win_length, n_fft,
              ...
              noise_margin_db=6.0, min_prominence_db=5.0,
              transition_weight=0.01, harmonic_bonus=5.0,
              harmonic_tolerance=0.15, n_harmonics_check=4,
              freq_boost_exp=1.0, smooth=True,
              jump_threshold_hz=500, smooth_window=5):
    """
    ...
    mag_db, freqs, times, peaks_list, vmask_aligned, f_valid = find_spectral_peaks_prominence(
        y=y, vmask=vmask, fs=fs, hop_length=hop_length, win_length=win_length, n_fft=n_fft,
        f0_min=f0_min, f0_max=f0_max, noise_margin_db=noise_margin_db,
    ...
    for peak_idx in range(len(peak_freqs)):
        ...
        freq_boost_db = freq_boost_exp * 20.0 * np.log10(f0_max / curr_freq)
        emission_score = emission_score + freq_boost_db + bonus
...
def estimate_amplitude(y, fs, win_length, hop_length, n_fft, f_min=200.0, f_max=8000.0):
...
brel_grid = 1.0 - np.logspace(np.log10(1 - brel_max), 0, n)

with open(files("vibe") / "a_b_sweep.pkl", "rb") as f:
...
class BiomechanicalSongFitter:
    def __init__(self, a_grid, brel_grid, pitch_grid, amplitude_grid):
        self.a_grid = a_grid
        self.brel_grid = brel_grid
    ...
    def extract_isopitch_contour(self, target_pitch):
        ...
        cs = ax.contour(self.a_grid, self.brel_grid, self.pitch_grid.T, levels=[target_pitch])
        plt.close(fig)
        ...
        brel_contour = contour_vertices[:, 1]
        
        return a_contour, brel_contour
    
    def get_amplitude_along_contour(self, a_contour, brel_contour):
        points = np.column_stack([a_contour, brel_contour])
    ...
    def _extract_contour_worker(self, t, pitch):
        a_cont, brel_cont = self.extract_isopitch_contour(pitch)
        amp_cont = self.get_amplitude_along_contour(a_cont, brel_cont)
        return t, a_cont, brel_cont, amp_cont
    ...
    def fit_song(self, birdsong_pitch, birdsong_amplitude, voicing_mask,
        ...
        contours = {}
        amplitude_contours = {}
        for t, a_cont, brel_cont, amp_cont in contour_results:
            contours[t] = (a_cont, brel_cont)
        ...
        for idx, t in enumerate(voiced_indices):
            ...
            brel_traj[t] = (1 - weight) * brel_cont[idx_low] + weight * brel_cont[idx_high]
    ...
    def plot_fit(self, birdsong_pitch, birdsong_amplitude, fit_result, voicing_mask, time=None):
        ...
        ax = axes[2]
        ax.plot(time, fit_result['beta'], linewidth=2, color='orange')
        ax.set_ylabel('β')
        ax.set_xlabel('Time')
        ax.set_xlim([min(time), max(time)])
        ax.invert_yaxis()
    ...
    def plot_parameter_heatmap(self, fit_result, voicing_mask=None):
        ...
        brel_traj = np.asarray(fit_result['beta'])
        
        if voicing_mask is not None:
            voicing_mask = np.asarray(voicing_mask)
            alpha_voiced = alpha_traj[voicing_mask]
            brel_voiced = brel_traj[voicing_mask]
        else:
            alpha_voiced = alpha_traj
            brel_voiced = brel_traj
        
        fig, ax = plt.subplots(figsize=(7, 5))
        im = ax.contourf(self.a_grid, self.brel_grid, self.amplitude_grid.T, 
                         levels=20, cmap='inferno')
        time_indices = np.arange(len(alpha_voiced))
        scatter = ax.scatter(alpha_voiced, brel_voiced, c=time_indices, 
                             s=5, cmap='cool')
        ax.set_xlabel('α')
        ax.set_ylabel('β (relative distance to SNILC cusp)')
...
def interpolate_nan_pitch(pitch, vmask):
    ...
    for idx in nan_voiced:
        ...
        if left_valid is not None and right_valid is not None:
            pitch[idx] = (left_valid + right_valid) / 2.0  # average neighbors
        elif left_valid is not None:
            pitch[idx] = left_valid  # copy if only left neighbor
        elif right_valid is not None:
            pitch[idx] = right_valid  # copy if only right neighbor
        else:
            vmask[idx] = False  # if isolated frame, mark as unvoiced
    
    return pitch, vmask

def song_to_parameters(waveform, fs_audio, vmask, hop_length, win_length, n_fft, pitch_kwargs=None):
    """
    Extract biomechanical parameters from birdsong
    """
    ...
    song_pitch, vmask = interpolate_nan_pitch(song_pitch, vmask)
    ...
    # silence-trajectory parameters (fit using pressure data from Jorge Mendez)
    SILENCE_A = 0.5
    SILENCE_M = 0.194
    SILENCE_RAMP = 0.005
    SILENCE_DP = 5.424585123451652
    SILENCE_TAU = 0.02978554821402764
    
    def _rcos(u):  # raised-cosine 0->1, C1 at both ends
        return 0.5 * (1.0 - np.cos(np.pi * np.clip(u, 0.0, 1.0)))
    
    def _silence_segments_from_mask(vmask):
        """Contiguous unvoiced runs on the frame grid"""
        v = np.asarray(vmask, bool)
        idx = np.flatnonzero(~v)
        if idx.size == 0:
            return
        splits = np.flatnonzero(np.diff(idx) > 1) + 1
        for run in np.split(idx, splits):
            yield run[0], run[-1], run.size
    
    def apply_silence_traj_frames(alpha, vmask, fs, hop,
                                  A=SILENCE_A, m=SILENCE_M, dP=SILENCE_DP,
                                  tau=SILENCE_TAU, ramp_s=SILENCE_RAMP):
        """Set alpha during silence to a duration-dependent interior level with
        raised-cosine ramps blending from each voiced edge"""
        a = alpha.copy()
        for f0, f1, n_fr in _silence_segments_from_mask(vmask):
            d = n_fr * hop / fs
            a_int = A + m * dP * np.exp(-d / tau)
            idx = np.arange(f0, f1 + 1)
            if ramp_s <= 0 or f1 == f0:
                a[idx] = a_int
                continue
            t = (idx - f0) * hop / fs
            span = t[-1]
            a_start = alpha[f0 - 1] if f0 > 0 else a_int
            a_end   = alpha[f1 + 1] if f1 + 1 < len(alpha) else a_int
            g    = np.minimum(_rcos(t / ramp_s), _rcos((span - t) / ramp_s))
            edge = a_start + (a_end - a_start) * (t / span if span > 0 else 0.0)
            a[idx] = edge * (1.0 - g) + a_int * g
        return a

    fit_result['alpha'] = apply_silence_traj_frames(
        np.asarray(fit_result['alpha'], float), vmask, fs_audio, hop_length)
    
    return fit_result['alpha'], fit_result['beta'], fit_result

def synthesize_song(alpha, beta_or_beta_rel, fs_audio, len_targ, gain=None, 
                    steps_per_sample=8, integrator='heun', device=torch.device('cpu'),
                    sys_pars=None, v_sound=None, input_is_beta_rel=True):
    """
    ...
    - input_is_beta_rel: If True, convert β_rel (0-1 proportional distance) to β before synthesis
    ...
    if input_is_beta_rel:
        beta_rel = torch.as_tensor(beta_or_beta_rel, dtype=torch.float32)
        b_lo = -1/3 + 1e-3
        b_hi = snilc.upper_bound(alpha).detach()
        beta = b_lo + beta_rel * (b_hi - b_lo)
    else:
        beta = torch.as_tensor(beta_or_beta_rel, dtype=torch.float32)
